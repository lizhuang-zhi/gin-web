...
func main() {
	...
	var primeChan chan int = make(chan int, 450000) // 存储所有素数的管道
	var exitChan chan bool = make(chan bool, 4)     // 用于标记多个协程是否都执行完毕[这里越大并不会使速度越快, 因为这和计算机的CPU个数有关]
